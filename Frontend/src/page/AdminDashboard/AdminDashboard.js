...
function renderStats(data) {
    ...
    if (totalUsersDeltaEl)
        totalUsersDeltaEl.textContent = `${data.activeUsers} Active Users`;
    if (totalAssetsEl)
        totalAssetsEl.textContent = data.totalInvestments.toString();
    if (totalAssetsDeltaEl)
        totalAssetsDeltaEl.textContent = `Amount: $${data.totalInvestmentAmount.toFixed(2)}`;
    if (activeAssetsEl)
        activeAssetsEl.textContent = data.activeInvestments.toString();
    if (activeAssetsPercentEl)
        activeAssetsPercentEl.textContent = `Current Value: $${data.totalCurrentValue.toFixed(2)}`;
    if (categoriesCountEl)
        categoriesCountEl.textContent = `$${data.totalProfit.toFixed(2)}`;
    if (categoriesListEl)
        categoriesListEl.textContent = "Total Profit";
}
async function init() {
